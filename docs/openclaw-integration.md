...
### 问题 4：arXiv API 请求频率限制 (HTTP 429)

**症状**：运行脚本时出现 `错误: arXiv API 请求频率过高 (HTTP 429)` 错误

**解决方案**：

1. **等待一段时间后重试**
   - arXiv API 限制每 3 秒最多 1 次请求
   - 如果遇到频率限制，等待 30 秒到 1 分钟后重试
...
2. **使用缓存功能**（推荐）
   ...
   # 禁用缓存，强制获取最新结果
   python arxiv_search.py -d 2026-03-17 -l --no-cache
   ```

3. **减少关键词数量**
   - 在 `config.json` 中减少关键词数量
   - 避免使用过于宽泛的关键词
